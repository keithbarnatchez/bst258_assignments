...
\end{aligned}
$$
Notice $Y$ is independent at $A$ given $L$, so that for any $l$ the true conditional covariance is 0 and in turn the expected conditional covariance is 0.
...
1. Both estimators (e.g. GLM-based and SuperLearner-based) are centered around the true expected conditional covariance of 0.
2. The GLM-only estimator concentrates a bit faster
